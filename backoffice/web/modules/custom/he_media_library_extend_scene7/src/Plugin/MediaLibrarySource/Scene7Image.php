...
/**
 * Provides a media library pane to pull images from DAM Adobe Scene7.
 *
 * @MediaLibrarySource(
 *   id = "scene7_image",
 *   label = @Translation("Image Scene7"),
 *   source_types = {
 *     "image",
 *   },
 * )
 */
class Scene7Image extends MediaLibrarySourceBase {
  ...
  protected function queryResults() {
    ...
    $params = [
      ...
      'folder' => 'hermesdigital/hecate/image',
      'includeSubfolders' => TRUE,
      'assetTypeArray' => [
        'items' => ['AnimatedGif', 'Image', 'Video', 'AssetSet'],
      ],
    ] + $options;
  ...
  public function getEntityId($selected_id) {
    ...
    $url = 'https://assets.hermes.com/is/image/' . $media_assets->ipsImageUrl . '?name=' . $media_assets->name . '&end';

    // Create a media entity.
    $entity = $this->createEntityStub($media_assets->name . ' - ' . $selected_id);
    ...
    try {
      ...
      $filename = $selected_id . '_' . $media_assets->name . '.jpg';
      ...
      $entity->{$source_field}->alt = $media_assets->name;
      $entity->save();
